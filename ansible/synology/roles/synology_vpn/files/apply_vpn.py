...
# The chicken-and-egg ender. OUT_KEYS cannot be confirmed without reading a live
# Settings.Config payload; reading one needs a web session; the only place a DSM
# credential legitimately exists is CD (deploy-ansible.sh brokers
# nas_admin_password out of OpenBao). But enabling the config tasks in CD to get
# that reading would fail the deploy on the very mismatch we are trying to fix.
#
# So: a subcommand that ONLY reads, prints the key names, and ALWAYS exits 0.
# It cannot fail a deploy, so it is safe to run unattended (see "merging IS
# applying" in CLAUDE.md), and its stdout lands in the deploy log where it can
# be read without anyone handling a password.
#
# TEMPORARY. Once OUT_KEYS is reconciled, set synology_vpn_discover: false --
# there is no reason to keep logging into DSM on every deploy.

# Values whose names suggest secrets never get printed. The deploy log is not a
# secret store, and a schema dump only needs KEY NAMES to be useful.
...
def cmd_dump(a):
    """Print the live VPNServer schema. NEVER returns non-zero: this runs in an
    unattended pipeline and must not be able to break it.

    Output deliberately avoids the FAIL/CHANGED sentinel substrings the role
    greps for (see [[synology-script-fail-sentinel-pty-merge]]) -- a schema dump
    is neither a failure nor a change.
    """
    for api in (CONFIG_API, ACCOUNT_API):
        try:
            with _session(a) as sess:
                data, err = _load(sess, api)
            if err is not None:
