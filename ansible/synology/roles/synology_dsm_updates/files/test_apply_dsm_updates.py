...
def test_setting_no_change(monkeypatch, capsys):
    fake, _ = _factory(
        {m.UPD_SETTING_API: {"get": dict(_CONVERGED, schedule=dict(_CONVERGED["schedule"]))}}
    )
...
def test_setting_drift_applies_real_field_names(monkeypatch, capsys):
    ...
    set_call = next(p for a, v, mth, p in captured if a == m.UPD_SETTING_API and mth == "set")
...
def test_setting_uses_v2_not_v1(monkeypatch, capsys):
    ...
    fake, captured = _factory(
        {
            m.UPD_SETTING_API: {
                "get": dict(_CONVERGED, schedule=dict(_CONVERGED["schedule"], week_day="1"))
            }
        }
    )
    monkeypatch.setattr(m, "_exec", fake)
...
def test_setting_check_mode_no_apply(monkeypatch, capsys):
    drift_live = dict(_CONVERGED, autoupdate_enable=False, schedule=dict(_CONVERGED["schedule"]))
    fake, captured = _factory({m.UPD_SETTING_API: {"get": drift_live}})
    monkeypatch.setattr(m, "_exec", fake)
    rc = m.main(
        [
