...
def _geoip_probe_dispatcher(per_adapter_rules=None):
    ...
    def fake(api, *params):
        ...
        if api == "SYNO.Core.Security.Firewall.Adapter" and method == "list":
            return {"success": True, "data": {
                "adapter_names": ["eth0", "global"]}}
        if api == "SYNO.Core.Security.Firewall.Rules" and method == "load":
            adapter = next((p.split("=", 1)[1].strip('"')
                            for p in params if p.startswith("adapter=")), "")
...
def test_probe_geoip_counts_geoip_rules(monkeypatch, capsys):
    ...
    rules = [{"enabled": True, "service_policy": "allow",
              "set_type": "geoip", "src": "US"}]
    monkeypatch.setattr(m, "_exec",
                        _geoip_probe_dispatcher(per_adapter_rules={"eth0": rules}))
    rc = m.main(["probe-geoip"])
    assert rc == 0
    out = capsys.readouterr().out.strip()
    import json as _j
