...
def _activate_live(enabled=False):
    ...
    return {m.SSO_PROFILE_FIELD: m.ACTIVATE_PROFILE if enabled else "", m.SSO_ENABLE_FIELD: enabled}
...
def _oidc_applied():
    ...
    return _oidc_live(
        oidc_allow_local_user=True,
        oidc_client_id="e4e-nas",
        oidc_client_secret="s3cr3t",
        oidc_name="Authentik",
        oidc_redirect_uri="https://e4e-nas.ucsd.edu:6021",
        oidc_scope="openid profile email",
        oidc_user_claim="preferred_username",
        oidc_wellknown=_WELLKNOWN,
        oidc_authorization_endpoint=_AUTHZ,
        oidc_token_endpoint=_TOKEN,
    )
...
def test_oidc_no_change(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        m, "_exec", _exec_factory(_oidc_applied(), _activate_live(True), _setting_live(True))
    )
    rc = m.main(_argv())
...
def test_oidc_would_change_in_check_does_not_set(monkeypatch, capsys):
    ...
    sets = []
    monkeypatch.setattr(
        m, "_exec", _exec_factory(_oidc_live(), _activate_live(), _setting_live(), set_capture=sets)
    )
...
def test_secret_rotation_triggers_oidc_set_only(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        m, "_exec", _exec_factory(live, _activate_live(True), _setting_live(True), set_capture=sets)
    )
    rc = m.main(_argv())
    out = capsys.readouterr().out
...
def test_default_login_only_drift_sets_setting_only(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        "_exec",
        _exec_factory(
            _oidc_applied(), _activate_live(True), _setting_live(False), set_capture=sets
        ),
    )
    rc = m.main(_argv())
    assert rc == 0 and capsys.readouterr().out.startswith("CHANGED")
...
def test_activation_only_drift_enables_service(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        "_exec",
        _exec_factory(
            _oidc_applied(), _activate_live(False), _setting_live(True), set_capture=sets
        ),
    )
    rc = m.main(_argv())
    assert rc == 0 and capsys.readouterr().out.startswith("CHANGED")
