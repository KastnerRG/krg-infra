...
def test_admin_grants_no_change_when_already_present(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        "_run",
        lambda cmd: (
            _R(_list_acl_with_rw(["@administrators", "@KRG\\Domain Admins"]))
            if "--list_acl" in cmd
    ...
    rc = m.main(
        [
            "admin-grants",
            "--shares",
            json.dumps(["admin", "maya"]),
            "--grants",
            json.dumps(["@KRG\\Domain Admins"]),
            "--tier",
            "RW",
        ]
    )
...
def test_admin_grants_additive_only_for_missing(monkeypatch, capsys):
    ...
    assert csv == "@KRG\\Domain Admins", "must only `+`-add MISSING grants; instead added " + csv
...
def test_admin_grants_check_mode_writes_nothing(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        "_run",
        lambda cmd: (
            _R(_list_acl_with_rw(["@administrators"]))
            if "--list_acl" in cmd
    ...
    rc = m.main(
        [
            ...
            json.dumps(["admin"]),
            "--grants",
            json.dumps(["@KRG\\Domain Admins", "@KRG\\E4E Admin"]),
            "--tier",
            "RW",
            "--check",
        ]
...
def test_admin_grants_never_touches_other_tiers(monkeypatch, capsys):
    ...
    for cmd in calls:
        ...
        assert cmd[tier_pos] == "RW", "admin-grants must only touch --tier; saw " + cmd[tier_pos]
...
def test_admin_grants_rejects_bad_tier():
    ...
    with pytest.raises(SystemExit):
        m.main(["admin-grants", "--shares", "[]", "--grants", "[]", "--tier", "BOGUS"])
...
def test_recursive_stamp_check_mode(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr(
        m,
        "_run",
        lambda cmd: (_ for _ in ()).throw(AssertionError("must not invoke synoacltool in --check")),
    )
