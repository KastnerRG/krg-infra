...
def test_main_no_change(monkeypatch, capsys):
    ...
    rc = m.main(
        [
            ...
            "15",
            "--categories",
            json.dumps(["system_health", "security"]),
            "--notify-email",
            "true",
        ]
    )
...
def test_main_strips_success_but_keeps_scheduleTaskId(monkeypatch, capsys):
    ...
    capsys.readouterr()
    set_call = next(p for a, p in captured if a == m.SA_CONF_API and "method=set" in p)
    # `success` stripped
...
def test_all_seven_weekdays_map_correctly(monkeypatch, capsys):
    ...
    for spec_day, dsm_str in expected.items():
        ...
        capsys.readouterr()
        set_calls = [
            p for a, p in captured if a == m.SA_CONF_API and any("method=set" in s for s in p)
        ]
        if spec_day == "Sun":
            # no drift -> no SET
            ...
            assert payload["weekday"] == dsm_str, spec_day + " should map to weekday=" + dsm_str
