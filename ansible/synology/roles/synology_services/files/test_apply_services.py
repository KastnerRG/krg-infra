...
def test_ftp_no_change(monkeypatch, capsys):
    monkeypatch.setattr(
        m, "_exec", _exec_factory({"enable_ftp": False, "enable_ftps": False, "ext_ip": ""})
    )
...
def test_snmp_v3_apply_with_creds(monkeypatch, capsys):
    ...
    for tok in (
        ...
        'location="UCSD"',
        'v3_auth_proto="SHA"',
        'v3_auth_passwd="secret-auth-pw-min8"',
        'v3_priv_proto="AES"',
        'v3_priv_passwd="secret-priv-pw-min8"',
    ):
