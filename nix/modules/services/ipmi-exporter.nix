...
with lib; let
  ...
  # Re-assert /dev/ipmi0 perms as root before the (non-root) exporter starts.
  # The udev rule below sets GROUP/MODE at device creation, but `nixos-rebuild
  # switch` only RELOADS udev rules — it does not re-trigger already-present
  # devices, so on a live deploy the device keeps its 0600 root:root perms until
  # the next reboot. This makes the fix take effect immediately on deploy (and
  # self-heals if the device ever reappears with default perms). Guarded so it
  # no-ops on a host with no in-band BMC.
  ipmiDevPerms = pkgs.writeShellScript "ipmi-dev-perms" ''
    if [ -e /dev/ipmi0 ]; then
      ${pkgs.coreutils}/bin/chgrp ipmi /dev/ipmi0
      ${pkgs.coreutils}/bin/chmod 0660 /dev/ipmi0
    fi
  '';
in {
  ...
  config = mkIf cfg.enable {
    ...
    #      exporter's dynamic user to it (DAC). A udev rule sets that at device
    #      creation; ExecStartPre (ipmiDevPerms) re-asserts it on live deploys
    #      where udev rules reload but already-present devices aren't retriggered.
    ...
    systemd.services.prometheus-ipmi-exporter.serviceConfig = {
      ...
      # "+" → run as root despite the unit's DynamicUser (it has no privilege to
      # chgrp the device itself).
      ExecStartPre = ["+${ipmiDevPerms}"];
    };
