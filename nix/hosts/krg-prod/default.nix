...
in {
  ...
  # GitHub + Slack integration pending (see GitHub issue #74) — the two bots are
  # parked behind a compose profile by the override below so they don't crash-loop
  # on missing tokens and fail the deploy; only backend + db run until #74 lands.
  krg.composeStacks.e4e-roster = {
    description = "E4E Roster V3 — backend + postgres";
    composeFiles = [
      "/var/lib/krg/e4e-roster/docker-compose.yml"
      "${../../docker-compose/e4e-roster/disable-bots.override.yml}"
    ];
