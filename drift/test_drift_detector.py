...
def _webapi_lines(data):
    ...
    return ["[Line 1] Exec WebAPI:  api=X, method=get, param={}"] + json.dumps(
        obj, indent=3
    ).splitlines()
...
def test_parse_enum_shares_and_groups():
    ...
    assert d.parse_enum(["4 Group Listed:", "administrators", "users"]) == [
        "administrators",
        "users",
    ]
...
def test_diff_smb_drift_and_clean():
    ...
    assert {x["key"] for x in d.diff_smb(spec, drift_snap)} == {
        "smb_min_protocol",
...
def test_diff_nfs_global_and_rules():
    spec = {
        "nfs": {"enable": True, "nfsv4": True, "v4_domain": ""},
        "exports": [{"share": "s", "rules": [RULE]}],
    }
    snap = {
        "nfs_global_raw": _webapi_lines(
            {"enable_nfs": False, "enable_nfs_v4": True, "nfs_v4_domain": ""}
        ),
    ...
    clean = {
        "nfs_global_raw": _webapi_lines(
            {"enable_nfs": True, "enable_nfs_v4": True, "nfs_v4_domain": ""}
        ),
...
def test_diff_acls():
    ...
    snap = {
        "share_acls": [{"share": "maya", "list_acl": ACL_LINES}]
    }  # @maya,bob live vs @maya desired
...
def test_run_reports_per_resource_and_metrics(tmp_path):
    ...
    (snap_dir / "e4e-nas-shares.yml").write_text(
        yaml.safe_dump({"shares_raw": ["1 Listed:", "maya", "homes"]})
    )
    (snap_dir / "e4e-nas-smb.yml").write_text(
        yaml.safe_dump(
            {
                "smb_raw": _webapi_lines(
                    {
...
def test_missing_snapshot_is_check_failure(tmp_path):
    ...
    snap_dir.mkdir()
    _write_specs(
        spec_dir, **{"shares.yml": {"shares": [{"name": "maya"}]}}
    )  # but no shares snapshot
    results, _ = d.run(str(spec_dir), str(snap_dir), "e4e-nas")
    assert results["shares"] == "no-snapshot"
    assert 'krg_synology_drift_check_success{host="e4e-nas"} 0' in d.render_prometheus(
        "e4e-nas", results, []
    )
...
def test_main_exit_codes(tmp_path, capsys):
    ...
    (snap_dir / "e4e-nas-shares.yml").write_text(
        yaml.safe_dump({"shares_raw": ["1 Listed:", "maya"]})
    )
